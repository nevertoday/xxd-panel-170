...
# XXD Panel 170｜검정과 녹슨 빨강의 거친 작은 삽화
...
작은 형태에 큰 건성 붓질을 씁니다. 숯검정이 주도하고 녹슨 빨강은 소량 강조하며 배경 색판과 다색 채색은 금지합니다.
...
- `design-only`: 전체 캔버스에 Panel 170의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-170`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-170 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-170-prompt.en.md)와 [중국어](references/xxd-panel-170-prompt.zh-CN.md)를 확인하세요.
