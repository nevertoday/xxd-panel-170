...
異なる元画像から個別に生成した Panel 170 の初回作例8点です。短い英文を使用。横長は1536×864の左右等分、縦長は1152×1536の上下等分です。

元画像との対応、黒と錆赤の乾いた筆致、紙面を各画像で確認しました。一部の被写体や文字が大きく、05・06は細部過多、09・12は風景を残しすぎ、10は赤い太陽を追加、11は赤が目立ちます。小さな独立挿絵、最少の細部、広大な余白をすべて満たしたわけではありません。初回結果の記録であり、全点合格の見本ではありません。
...
小さな図形に大きな乾性筆致。炭黒が主役で錆赤は少量の差し色、背景の色板や多色化は禁止。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 170 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-170` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-170 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-170-prompt.en.md)／[中国語](references/xxd-panel-170-prompt.zh-CN.md)を参照してください。
